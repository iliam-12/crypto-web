...
const Home = () => {
  ...
  const [cryptoList, setCryptoList] = useState<Array<CryptoObject>>([]);
  ...
  useEffect(() => {
    ...
    socket.onmessage = function (event) {
      ...
      setMyCryptoList([]);
      cryptoList.map(async (crypto: { name: string }) => {
        if (crypto.name === data.s.substr(0, 3) && cryptoListIndex < cryptoList.length) {
          console.log('check : ' + crypto.name);
          newCryptoList.push({
            ...crypto,
            name: crypto.name,
          ...
          newCryptoList.map(async (existingCrypto: any) => {
            if (existingCrypto.name === data.s.substr(0, 3)) {
              ...
              existingCrypto.imageUrl = await getCryptoImgUrl(existingCrypto.name);
            }
          });
        }
      });
      const cryptoListNames = cryptoList.map((crypto) => crypto.name);
  ...
  useEffect(() => {
    const updatedCryptoList = cryptoList.map(async (crypto: { name: string }) => {
      return {
        ...crypto,
